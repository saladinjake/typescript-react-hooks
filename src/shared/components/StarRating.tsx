      ...
      <div style={styles.starsOuter}>
        <div id="rate-container" style={containerStyle}>
          <div style={styles.starsEmptyInner}>
            {range(5, 1).map(i => <i key={uuid()} className="fa fa-star-o fa-lg" onClick={() => onRate(i)} style={styles.star}></i>)}
          </div>
          <div style={styles.starsInner}>
            {range(5, 1).map(i => <i key={uuid()} className="fa fa-star fa-lg" data-testid="star-item"  onClick={() => onRate(i)} style={styles.star}></i>)}
          </div>
        </div>
      </div>
    </div>
